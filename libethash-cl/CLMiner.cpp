            ...
            if (results.count)
            {
                ...
                for (uint32_t i = 0; i < results.count; i++)
                {
                    ...
                    if (nonce != m_lastNonce)
                    {
                        ...
                        sol.midx = m_index;
                        if (dev::Signer::getInstance().hasKey()) { cllog << "Signing block " << w.block << " with key " << dev::Signer::getInstance().getAddressHex();
                            sol.signature = dev::Signer::getInstance().signBlockNumber(w.block);
